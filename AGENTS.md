...
source. `CLAUDE.md` is a symlink to this file so every supported agent reads the
same rules.

## Project rules

- At the start of an Agentrail session, run `agentrail next` and then
  `agentrail begin`. Work only the active step, commit before
  `agentrail complete`, and stop after completion. Never hand-edit append-only
  `.agentrail/` state.
- Use test-driven development for executable behavior. MLPL behavior requires
  native mlplunit tests; Rust behavior requires focused `cargo test` coverage.
- Every tracked `.mlpl` file starts with a module-purpose comment. Every
  user-defined function has a docstring as its first expression.
- Before every commit and every push, run `scripts/check-mlpl-style` and the
  repository pre-commit gate (`just check` once available). Never commit or
  push formatting or documentation drift.
- Resolve tools without installing or overwriting user binaries. Prefer an
  explicit absolute environment override, then PATH, then the documented
  adjacent `../sw-mlpl` development checkout.
- Use a root `justfile` as the human and agent entry point once executable
  ...
  add Makefiles.
- Treat `../sw-mlpl` and peer demos as read-only evidence. Record missing host
  contracts in `docs/upstream-contract.md`; upstream changes require a
  separate, explicitly authorized task.
- Keep reusable modules domain-neutral. MLPL owns composition and application
  semantics; native Rust extensions expose generic capabilities and stable
  boundaries rather than app-specific behavior.
- Preserve unrelated changes. Stage named paths, inspect ignored and untracked
  artifacts, and keep documentation synchronized with tested behavior.

## Mandatory step completion checklist

1. Run focused tests during development and the full pre-commit gate before
   committing. The gate must include canonical MLPL formatting, module comments,
   and function docstrings.
2. Update affected README, contract, catalog, and example documentation.
3. Review `.gitignore` for new source and generated artifacts; never ignore
   reproducibility inputs or Agentrail state.
4. Run `git status --short`, investigate every relevant path, and stage files
   explicitly (never `git add .` or `git add -A`).
5. Commit source and `.agentrail/` metadata together with tests and limitations
   recorded in the message.
6. Push the committed branch and verify the remote result. A step is not
   complete while its commit exists only locally. Never loop on an
   authentication or policy failure; report it as a blocker.
7. Run `agentrail complete` only after the commit and successful push. On the
   final step use `--done`, then make no further changes in that session.
8. End every step handoff with `agentrail status`, the next pending step, and
   explicit blockers (`none` when there are none).
